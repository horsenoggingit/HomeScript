...
// This means clients can specify their own key or have one generated.
//
...
class AccessoryTrackedGetterScripter: NSScriptCommand {
    ...
    @objc public override func performDefaultImplementation() -> Any? {
        ...
        // the user can specify their own id or one will be autogenerated if an id is not passed in.
        // The response will echo the key passed in.
        if arguments["id"] as? String == nil || AccessoryTrackedGetterScripter.continuationStore[client] == nil {
            
            if arguments["id"] == nil {
                AccessoryTrackedGetterScripter.logger.info("New client with autogenerated id \(client)")
            } else {
                AccessoryTrackedGetterScripter.logger.info("New client with id \(client)")
            }
